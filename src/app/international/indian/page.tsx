...
export default function IndianCurriculumPage() {
    const { t } = useLanguage();
    const { data: pageData } = usePageData('international/indian');

    return (
        <Layout>

            <MetaSEO 
                seo={pageData?.seo || { metaTitle: 'Indian (CBSE) Curriculum | Ivy Bridge Elite Tutoring', metaDescription: 'Expert 1-on-1 tutoring for the Indian CBSE curriculum at Ivy Bridge. Personalized support for school exams and entrance prep.' }} 
                defaultTitle="Indian (CBSE) Curriculum | Ivy Bridge" 
            />
            <section className="pt-4 md:pt-8 pb-10 md:pb-16 bg-white border-t border-slate-100">
                <div className="max-w-7xl mx-auto px-6">
                    <div className="text-center mb-10 md:mb-24 flex flex-col items-center">
                        <img src="/cbse-logo.png" alt="CBSE" className="h-14 md:h-32 mb-8 object-contain" />
                        <h2 className="text-2xl xs:text-3xl sm:text-4xl md:text-7xl font-black uppercase mb-4 leading-tight">
                            <span className="text-navy">Indian</span> <span className="text-[#fb923c]">CBSE</span> <br />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 p-1 xs:p-4 md:p-8">
                        {[
                            {
                                name: 'Science & Math',
                                items: ['Mathematics (Standard/Basic)', 'Physics', 'Chemistry', 'Biology', 'Integrated Science']
                            },
                            {
                                name: 'Commerce & Humanities',
                                items: ['Accountancy', 'Business Studies', 'Economics', 'History', 'Geography', 'Political Science']
                            },
                            {
                                name: 'Language & Extras',
                                items: ['English Core/Elective', 'Hindi / Sanskrit', 'Computer Science (Python)', 'Information Practices', 'Physical Education']
                            }
                        ].map((group, i) => (
                                ...
                                <div className="space-y-4">
                                    {group.items.map((item, j) => (
                                        <div key={j} className="flex items-center gap-3 px-4 py-3 bg-white/60 backdrop-blur-md rounded-2xl border border-white/40 text-slate-700 font-black text-[14px] xs:text-[16px] md:text-[18px] hover:bg-white hover:shadow-lg hover:scale-[1.02] transition-all">
                                            <div className="w-2 h-2 rounded-full bg-[#fb923c] shrink-0" />
                                            {item}
                                        </div>
                                    ))}
                                </div>
