...
export default function SingaporeanCurriculumPage() {
    const { t } = useLanguage();
    const { data: pageData } = usePageData('international/singaporean');

    return (
        <Layout>

            <MetaSEO 
                seo={pageData?.seo || { metaTitle: 'Singaporean Curriculum | Ivy Bridge Elite Tutoring', metaDescription: 'Master the world-leading Singaporean curriculum at Ivy Bridge. Expert 1-on-1 tutoring for Math and Science.' }} 
                defaultTitle="Singaporean Curriculum | Ivy Bridge" 
            />
            <section className="pt-4 md:pt-8 pb-10 md:pb-16 bg-white border-t border-slate-100">
                <div className="max-w-7xl mx-auto px-6">
                    <div className="text-center mb-10 md:mb-24 flex flex-col items-center">
                        <img src="/singapore-logo.png" alt="Singapore Curriculum" className="h-14 md:h-32 mb-8 object-contain" />
                        <h2 className="text-2xl xs:text-3xl sm:text-4xl md:text-7xl font-black uppercase mb-4 leading-tight">
                            <span className="text-navy">Singapore</span> <span className="text-blue-700">Math & Science</span> <br />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 p-1 xs:p-4 md:p-8">
                        {[
                            {
                                name: 'Mathematics (CPA)',
                                items: ['Primary Mathematics', 'Secondary (Fast Track)', 'H1/H2 Mathematics', 'Integrated Math', 'Statistics']
                            },
                            {
                                name: 'Sciences (Inquiry)',
                                items: ['Physics (Theory/Practical)', 'Chemistry (Bonding/Organic)', 'Biology (Molecular/System)', 'Integrated Science', 'Practical Lab Prep']
                            },
                            {
                                name: 'Languages & Humanities',
                                items: ['English (Language/Lit)', 'Social Studies', 'History / Geography', 'General Paper (A-Level)', 'Economics']
                            }
                        ].map((group, i) => (
